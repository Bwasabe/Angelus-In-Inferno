...
public class EnemyPurple : EnemyMove
{
    ...
    private void JudgeLR(){
        if(transform.localPosition.x<_gameManager.MinPosition.x +0.3f){
            isLeft = false;
            isRight = true;
        }
        if(transform.localPosition.x>_gameManager.MaxPositon.x -0.3f){
            isRight = false;
            isLeft = true;
        }
    }
    ...
    private void SpawnOrInstantiate()
    {
        ...
        {
            enemyBullet = _gameManager.PoolManager.enemyBullet.transform.GetChild(0).gameObject;
    ...
    protected override void Despawn()
    {
        ...
        enemyHpBar.value = 1f;
        col.enabled = true;
        transform.SetParent(_gameManager.PoolManager.enemyPurplePool.transform, false);
